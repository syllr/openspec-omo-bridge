...
为示例项目 `add-user-auth` 添加用户认证能力,支持 OAuth2 (Google) + Email/Password 两种登录方式,产出可运行的最小可行认证模块。

---

## 2. Context

当前项目无任何认证模块,所有路由都是公开的。`add-user-auth` 是该项目的第一个认证特性,需要从零搭建 session 管理 + 密码哈希 + OAuth2 回调流程。考虑到这是 1.0 升级到 2.0 的首个真实业务 change,本示例同时也是 omo-spec 2.0 体系的 dogfooding 案例。
...
- **Must Have**:
  - 4 个 OpenSpec artifacts 通过 `openspec validate add-user-auth`(proposal + design + specs + tasks)
  - 1 份 compile plan 满足 OMO 9 章节格式
  - 每个 Wave 完成后停下等用户 review(per-wave-pause policy)
- **Must NOT Have**:
  - 不实施任何代码改动(source plan 阶段不写代码)
  - 不创建 `research/` 目录(proposal 阶段不输出 `[NEEDS INVESTIGATION]` 标记)
  - 不调 `openspec instructions` / `openspec status`(消除动态 fetch)
...
- **Artifact 结构验证**:
  - 每个 artifact 包含模板必需 sections(详见第 7/8 章)
  - `openspec validate add-user-auth` 通过(specs 阶段后)
  - `proposal.md` 含 1 个 New Capability:`user-auth`
  - `specs/user-auth/spec.md` 含至少 1 个 requirement + scenario
- **Compile Plan 验证**:
  - 满足 OMO 9 章节格式(第 9 章描述生成规则)
  - 每个 task 含 Acceptance Criteria(可执行命令)
  - Final Verification Wave 至少 3 个 FVW 项
- **Review Checkpoint**:
  - 每个 Wave 完成后停下让用户 review(per-wave-pause policy)
  - 全部完成后用户跑 `openspec validate add-user-auth` 再次确认
...
- **Sequential Constraints**:
  ...
  - Wave 2 (design) 依赖 Wave 1 输出(读取 Capabilities)
  - Wave 3 (specs) 依赖 Wave 1 + Wave 2 输出(每个 Capability 1 个 spec 文件)
...
- **Per-Wave Pause Policy**:
  - Wave 完成后,LLM 用 `question` 工具问"是否继续 Wave N+1?"
  - 用户 Yes → 继续 / No → 停下
  - 全部 4 Wave 完成后,提示用户跑 `/omo-apply-change add-user-auth`
...
> **本节由 LLM 填充"翻译规则"的指引**——9.1 翻译映射表 + 9.3 验证清单 + 9.4 不做的事 是模板写死的(可读但不要重写)。LLM 仅需在 9.2 写入位置(本例:`.omo/plans/add-user-auth.md`)之后追加任何额外的翻译备注(如有)。
