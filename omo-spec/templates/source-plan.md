...
specDir: spec/{{CHANGE_NAME}}
planFile: spec-source-{{CHANGE_NAME}}.md
...
> **本文件由 `omo-spec` skill 生成,跑 `/start-work spec-source-{{CHANGE_NAME}}` 执行。**
> 生成器版本:omo-spec 1.0
...
<!--
  ...
  用户 review 完本 source plan 后,跑:
    /start-work spec-source-{{CHANGE_NAME}}
  ...
  全部 Wave 完成后:
    ...
    2. 提示用户跑 /openspec-archive-change {{CHANGE_NAME}} 归档
-->
