...
| `tools/omo-spec.ts`            | 单文件 2 tool：`sync_tasks_from_plan` / `validate_omo_plan` + 3 纯函数 + 类型定义。                   |
...
测试用 `import { parseOmoPlan, generateOpenSpecTasks, validateOmoPlan, resolveChangeContext } from "../omo-spec"` 复用纯函数。`sync_tasks_from_plan` 直接读写文件；`validate_omo_plan` 校验 plan 结构。Oracle 验证上下文不再走 tool,Oracle agent 通过 `openspec instructions apply` 的 `contextFiles` + 自己跑 `git diff --name-only HEAD` 拿变更文件(避免 Bun.spawn 跨运行时依赖)。
