...
| `tools/omo-spec.ts`            | 单文件 3 tool：`sync_tasks_from_plan` / `validate_omo_plan` / `validate_plan_completion` + 4 纯函数 + 类型定义。 |
...
| `omo_spec_validate_plan_completion` | 审计 plan 完成度：统计 TODOs + Success Criteria 的 completed / pending 数量 + allDone 标志 | `{ change_name?, plan_file_path? }` — 至少传一个                          |
...
- `validate_omo_plan`:两个参数都可选,**至少传一个**,否则 throw
...
测试用 `import { parseOmoPlan, generateOpenSpecTasks, validateOmoPlan, validatePlanCompletion, resolveChangeContext } from "../omo-spec"` 复用纯函数。`sync_tasks_from_plan` / `validate_plan_completion` 直接读写文件；`validate_omo_plan` 校验 plan 结构。Oracle 验证上下文不再走 tool,Oracle agent 通过 `openspec instructions apply` 的 `contextFiles` + 自己跑 `git diff --name-only HEAD` 拿变更文件(避免 Bun.spawn 跨运行时依赖)。
